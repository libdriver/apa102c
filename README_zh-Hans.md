...
APA102C为三色RGB LED调光控制串接IC，采用CMOS工艺，提供三色RGB LED输出驱动器，调节输出256级灰度和32级亮度，APA102C具有两个控制管脚，CLK提供时钟为芯片级联提供同步信号。

LibDriver APA102C是LibDriver推出的APA102C全功能驱动，该驱动提供LED写入等功能并且它符合MISRA标准。
...
/src目录包含了LibDriver APA102C的源文件。

/interface目录包含了LibDriver APA102C与平台无关的SPI总线模板。

/test目录包含了LibDriver APA102C驱动测试程序，该程序可以简单的测试芯片必要功能。

/example目录包含了LibDriver APA102C编程范例。

/doc目录包含了LibDriver APA102C离线文档。

/datasheet目录包含了APA102C数据手册。
...
在线文档: [https://www.libdriver.com/docs/apa102c/index.html](https://www.libdriver.com/docs/apa102c/index.html)。
